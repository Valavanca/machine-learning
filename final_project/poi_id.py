...
skb_short = SelectKBest()
skb_short.fit(features_train,labels_train)

print "\n ********** Select K-Bests **********"
for i, s in enumerate((skb.scores_)):
    print " [", s, "] ", features_list[i+1]

print "\n __________________________________short"
for i, s in enumerate((skb_short.scores_)):
    print " [", s, "] ", features_list[i+1]
...
cv = StratifiedShuffleSplit(labels_train, n_iter=8, random_state = 42)
...
print "Best params Adaboost: ", gs.best_params_
